  ...
  try{
    ...
    for (const ev of events){
      try{
        ...
        else if (competitor.score != null && comp.competitors){ const other = comp.competitors.find(c=>c !== competitor); if (other && other.score != null) lastResults.push(Number(competitor.score) > Number(other.score) ? 'W' : 'L'); }
...
(async ()=>{
  ...
  const outPath = path.join(outDir, 'currentPick.json');
